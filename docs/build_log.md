...
## 2026-08-28 — STATE OF THE DEVICE (end-of-stage hardening audit — the honest version)

**What works, as measured.** The full loop runs on-device with no network: two chest mics through one USB receiver → per-channel VAD → cross-channel ratio arbitration (±6 dB) with anti-feedback gate, push-to-talk, overlap deferral → three-engine ASR (Parakeet for English; SenseVoice for zh/ja/ko; whisper, language-pinned, for everything else) → opus-MT for the 45 installed directed pairs with NLLB-200 as universal fallback → Piper/Supertonic voices → hard-panned stereo into two earbuds. Latency, real-mic measured: short English dialogue turns ≈ 2.5 s stop-to-audio on opus pairs; Spanish (whisper) turns ≈ 3–4 s; NLLB-fallback pairs 3.5–5 s per sentence; monologues chunk every ~4 s so translation starts mid-speech. The architectural floor is ≈ 2.0–2.2 s, most of it deliberate policy (0.7 s endpoint, ≥1.0 s cancel window). Recovery is live-proven, not theoretical: the Bluetooth ladder reconnected the earbuds through hundreds of real attempts today; the worker watchdog detected a killed ASR thread, rebuilt it, and was decoding again in ~8 s; a second death escalated to a full pipeline restart; the backlog warning no longer lies when a fault explains it. Autostart survived a real power cycle: kernel boot to READY in **46 seconds**, including one by-design retry while the compositor came up.

**Language coverage, honestly.** Eight languages are bench-verified on this hardware (zh ja en es pt de ru fr), Korean is effectively a ninth. The other 42 in the picker carry published-benchmark WER estimates, not measurements, and the tail fails *confidently* — fluent wrong text. The picker language is a hard contract: speech in the wrong language decodes as confident phonetic junk in the configured language, and a code-switched word inside an English sentence gets anglicized ("sí" → "C", live).

**What is fragile, by evidence.** Simultaneous speech works when both voices are strong on their own mics; the weak side of an overlap in the −6..0 dB band is still discarded by design, and a bystander's voice can be decoded (even twice) instead of ignored — the mute button remains the only bystander defense. Accented speech on the non-English side costs real accuracy and ~1.5–2 s over the old (language-unstable) engine. The capture clock shows unexplained +5..+66 s excursions vs wall under I/O load — every pipeline decision rides the sample clock so nothing misbehaves, but the mechanism is not understood, only monitored. The DSI panel silently failed to probe on one boot (ribbon reseated; the supervisor now announces it). One AirPods sink means one shared hardware volume and SBC joint stereo — measured clean by ear, still a compromise. WAV evidence keeps only each session's first 8 turns.

**Not yet built or verified, and it matters.** No read-only rootfs: a battery hard-cut can corrupt the SD card, and this device is a battery appliance — this is the top standing risk. EEPROM power flags unset (the DJI receiver runs inside the 600 mA USB cap — works, thin headroom). No RTC battery fitted, so fully-offline mode has no clock source. WiFi is still on. The UPS soak test under sustained ML load has not been run. The kiosk/cage boot path was designed but not built (the labwc desktop session hosts the UI). Unknown AirPods behaviors: one bud dying, true battery life. Known-and-tolerated bugs: the merge-hold deadline expires early (pre-decode timestamp), and the turn registry never prunes within a session.

## 2026-08-28 — preparing to go public

Two jobs: get the hardware identifiers out, and write documentation for someone who is not me.

The identifier sweep found the AirPods MAC, the DJI receiver's serial (inside the PipeWire node name), the hostname, the username, and the home path spread across the working tree — and, less conveniently, across 59 of the 60 commits in history. The working tree is now clean: per-device values live in a gitignored `device.json` created from a committed example, exactly the pattern v1 used for its `.env`, and the repo root is derived from `__file__` so nothing is tied to one machine. That last change fixed a bug rather than just a privacy issue — the receiver serial was hardcoded into the capture path, so a fresh clone would have failed on anyone else's hardware with no useful error.
